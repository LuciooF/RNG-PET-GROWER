...
function AutoHeavenButtonService:FindAutoHeavenButton()
    ...
    -- Find player's area (similar to how PetMagnetButtonService does it)
    local playerAreas = game.Workspace:FindFirstChild("PlayerAreas")
    if not playerAreas then
        warn("AutoHeavenButtonService: PlayerAreas not found")
        return
    end
    
    print("AutoHeavenButtonService: Found PlayerAreas, looking for player's area...")
    ...
    for _, area in pairs(playerAreas:GetChildren()) do
        if area.Name:match("PlayerArea") then
            print("AutoHeavenButtonService: Checking area:", area.Name)
            ...
            if nameplate then
                print("AutoHeavenButtonService: Found nameplate in", area.Name)
                local billboard = nameplate:FindFirstChild("NameplateBillboard")
                if billboard then
                    local textLabel = billboard:FindFirstChild("TextLabel")
                    if textLabel then
                        print("AutoHeavenButtonService: Nameplate text:", textLabel.Text, "Looking for:", player.Name .. "'s Area")
                        if textLabel.Text == (player.Name .. "'s Area") then
                            playerArea = area
                            print("AutoHeavenButtonService: Found player's area:", area.Name)
                            break
                        end
                    end
                end
            else
                print("AutoHeavenButtonService: No nameplate found in", area.Name)
            end
        end
    end
